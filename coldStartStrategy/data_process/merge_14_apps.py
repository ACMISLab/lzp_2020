...
for f_name in file_list:
    ...
    for ind, row in raw.iterrows():
        ...
        else:  # 如果是之前已有app，只需更新其调用时间序列与并发序列即可
            old_row_id = hashMap[row['HashApp']]
            newInvocTime = [1440 * days + int(i) for i in row['invoc_time_series'][1:-1].split(", ")]  # 按天修正分钟
...
resData.to_csv('./dataSet/appData_2019_14_days.csv', sep=';')
