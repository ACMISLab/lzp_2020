...
if Switch == 0:
    ...
    for episode in range(EP_MAX):
        ...
        for timestep in range(EP_LEN):
            ...
            reward_totle += reward
            reward = (reward - reward.mean()) / (reward.std() + 1e-5)  # 归一化？
            # PPO 更新
        ...
        if episode % 50 == 0 and episode > 100:  # 保存神经网络参数
            ...
            torch.save(save_data, ".\model\PPO2_all_model_actor.pth")
            save_data = {'net': critic.critic_v.state_dict(), 'opt': critic.optimizer.state_dict(), 'i': episode}
            torch.save(save_data, ".\model\PPO2_all_model_critic.pth")
    env.close()
    res = pd.DataFrame(all_ep_r)
    res.to_csv(".\model\PPO2_all_res_5.csv", header=False)
    ...
    aa = Actor()
    cc = Critic()
    checkpoint_aa = torch.load(".\model\PPO2_all_model_actor.pth")
    aa.old_pi.load_state_dict(checkpoint_aa['net'])
    checkpoint_cc = torch.load(".\model\PPO2_all_model_critic.pth")
